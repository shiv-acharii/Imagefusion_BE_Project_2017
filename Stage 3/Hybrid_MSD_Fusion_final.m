...
fprintf('\n ---------Hybrid MSD---------\n');
fprintf('\n%s\n%s\n',VIS,IR);
img1=imread(VIS);%read Vis image
img2=imread(IR);%read IR image 
[row1 col1] = size(img1);
[row2 col2] = size(img2);
fprintf('%d %d',row1,col1);
img1=imresize(img1,[256 256]);
img2=imresize(img2,[256 256]);

if(row1~=row2 || col1~=col2)
    error('input image are of different size');
end
...
fprintf('\nCorrelation coefficient of Visible image and fused image = %f\n',cr1);
fprintf('Correlation coefficient of IR image and fused image = %f\n',cr2);
